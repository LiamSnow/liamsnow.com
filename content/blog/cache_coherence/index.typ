...
= The MESI Protocol
MESI is a more advanced cache coherence protocol which enables write-back
and avoids wasted bus traffic.

MESI achieves this by having *each core* store
a small amount of state for *every #link-new-tab("cache line", "https://en.algorithmica.org/hpc/cpu-cache/cache-lines/")*.
...
Another system, MSI, exists -- which is the same, but without `Exclusive`.
MESI has `Exclusive` to reduce bus traffic:
...
Modern systems can't easily broadcast like in bus-based ones.
Instead, many use a *directory-based* approach:
