...
**NOTE**: This is one of my earlier Rust projects and has since been rewritten for [Igloo V2](https://github.com/LiamSnow/igloo/tree/main/esphome). The new version is cleaner and faster, but I wanted to keep this post up.

# Background

[ESPHome](https://esphome.io/) is customizable firmware for ESP microcontrollers, similar to Yocto but for smart home devices. You configure devices with YAML files, which generate C code that gets compiled and flashed to the microcontroller. Devices then communicate via a Protobuf-based protocol.

ESPHome devices were really only accessible through Home Assistant, which was created by the same team. The protocol was completely undocumented. The only existing library, [aioesphomeapi](https://github.com/esphome/aioesphomeapi), is written in Python and designed specifically for Home Assistant core.

I needed ESPHome support for [Igloo](https://liamsnow.com/igloo), which meant building the first implementation of the ESPHome protocol outside the Home Assistant ecosystem.

# Reverse Engineering

There was minimal prior work. [pixelspark](https://github.com/pixelspark) had created [esphome-rs](https://github.com/pixelspark/esphome-rs), but it had limited functionality. Fortunately, ESPHome uses Protobuf, which allowed me to automatically generate interface code.
...
Modern ESPHome devices use the Noise protocol for encrypted connections. I used the [snow](https://crates.io/crates/snow) crate, configuring it with the parameters `Noise_NNpsk0_25519_ChaChaPoly_SHA256` and prologue `NoiseAPIInit\x00\x00`.
