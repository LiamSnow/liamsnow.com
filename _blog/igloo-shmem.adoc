...
description: Designing ECS storage & queries for Igloo
written: 2026-02-23
updated: 2026-02-23
links: 
  - text: Igloo
...
Luckily, this is solved problem and has a few answers:
 ...
 . *Futex*: reader does a `FUTEX_WAIT` on a count and writer does `FUTEX_WAKE` after putting something new in.
   * This does not integrate with Tokio's https://en.wikipedia.org/wiki/Epoll[epoll] reactor at all
   * We could either drop async entirely (not really an option) or switch to a different runtime like https://github.com/bytedance/monoio[monoio] or https://github.com/DataDog/glommio[glommio] to use this.
 . https://man7.org/linux/man-pages/man2/eventfd.2.html[eventfd]: create an eventfd for each ring buffer we make
...
A nice part is that this has been done and in Rust https://github.com/diwic/shmem-ipc[shmem-ipc].
...
So let's get rid of it!
 ...
 . Distribute metadata (attached devices, names, groups, etc):
    * We want to optimize for the read-heavy workload. Sequential locks could work well here, but Rust also has a lot of good options (like https://lib.rs/crates/papaya[papaya]).
 ...
 . For bigger components (strings and vectors) we implement link:https://docs.kernel.org/locking/seqlock.html[seqlocks]:
   * Write:
...
This approach actually works great with the distributed device tree. I implemented a https://github.com/LiamSnow/igloo-device-tree-testing[PoC] and achieved much better performance than previous versions (1 billion QPS compared to 1 million). It's also just simpler.
...
== Conclusion
This was a great exploration. While not all paths were successful, two were (distributed device tree & column-wise bitset queries). We created a simpler and faster system, that has almost no bottlenecks. And I was able to prove this with the https://github.com/LiamSnow/igloo-device-tree-testing[PoC].
